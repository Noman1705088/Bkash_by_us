...
from .models import UserProfile, UpdateUser, AdminProfile,MerchantProfile
...
class HomeView(View):
    def get(self, request):
        if request.session.get('CUSTOMER'):
            ...
            if not (request.COOKIES.get('NAME') and request.COOKIES.get('PHOTO') and request.COOKIES.get('TRADE_LICENSE_NO') and request.COOKIES.get('HEAD_OFFICE_LOCATION')):
                ...
                resp.set_cookie('NAME', context['NAME'])
                resp.set_cookie('PHOTO', context['PHOTO'])
                resp.set_cookie('TRADE_LICENSE_NO',context['TRADE_LICENSE_NO'])
                resp.set_cookie('HEAD_OFFICE_LOCATION',context['HEAD_OFFICE_LOCATION'])
                return resp

            context = {'NAME': request.COOKIES.get('NAME'), 'PHOTO': request.COOKIES.get('PHOTO'),
                       'TRADE_LICENSE_NO': request.COOKIES.get('TRADE_LICENSE_NO'), 'TYPE': 'merchant', 'HEAD_OFFICE_LOCATION':request.COOKIES.get('HEAD_OFFICE_LOCATION')}
