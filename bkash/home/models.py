...
from dashboard.models import execute_sql,connection, USERS, hash_the_password, execute_sql_func
...
class AdminProfile:
    ...
    def addMoney(self,receiver_mobile_no,amount):
        ...
        if execute_sql(sql,[receiver_mobile_no],False,True,connection):
            receiver_id = execute_sql(sql,[receiver_mobile_no],False,True,connection)[0][0]
...
class AllAgentInfo:
    ...
    def getInfo(self):
        ...
        if execute_sql(sql, [], False, True,connection):
            agent = execute_sql(sql, [], False, True,connection)
            i = 0
            cont_agent = agent
            for x in agent:
                ...
                transaction = execute_sql_func(
                    'GET_TRANSACTION_COUNT_AGENT', [agent_id], False, True,connection)
        ...
        if execute_sql(sql, [], False, True,connection):
            ...
            for x in customer:
                ...
                transaction = execute_sql_func('GET_TRANSACTION_COUNT_CUSTOMER', [
                    customer_id], False, True,connection)
                tran = transaction.split()
    ...
    def getInfo(self):
        ...
        for x in merchant:
            ...
            merchant_info = execute_sql_func(
                'GET_MERCHANT_INFO', [merchant_id], False, True,connection)
...
class AllServiceProviderInfo:
    ...
    def getInfo(self):
        ...
        service_provider = execute_sql(sql, [], False, True,connection)
