...
class PayBillView(View):
    def get(self, request, service_id=0):
        if (request.session.get('CUSTOMER') or request.session.get('AGENT')) and service_id == 0:
            return render(request, 'services/payBill.html', {'PHOTO':request.COOKIES.get('PHOTO')})
...
class HistoryView(View):
    def get(self, request):
        ...
        if request.session.get('CUSTOMER'):
            ...
            i = 0
            for x in ans:
                cont_mar_his[i] = {'NAME_BRANCH': x[0], 'BRANCH_HIS': HistoryOf(
                    'MERC_'+str(x[1])).getHistory()}
                i = i+1
...
class MobileRechargeView(View):
    ...
    def post(self, request):
        ...
        mobile_recharge_trans = MobileRecharge(
            sender_id, receiver_mobile_no, recharge_amount, password, sender_type)
